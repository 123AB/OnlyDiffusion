	...
	if(!f1){
		...
		while(!f1.eof()){
            ...
            if(token.size() == 3){
                ...
                //cout << "String size 3 id is: " << id1 << endl;
                ...
                advance(it, 1);
                double value = std::stod(*it, NULL);
                //cout << "String size 5 id is: " << id1 << endl;
    ...
    if(seed_file != ""){
    	...
    	if(!f2){
    		...
    		while(!f2.eof()){
                ...
                //cout << "the algorithm start:" << endl;
    			for(int i=0; buffer[i] != '\0' && buffer[i] != '\n' && buffer[i] != '\r'; i++){
    				s += buffer[i];
                    string id = s;
                    User* seed = g->get_user(id, user_mapping);
                    //cout << "The s is:" << s << endl;
                    //cout << "the seed id is:" << id << endl;
        ...
        cout << "Finish reading seeds" << endl;
