...
FILEDIR = '../all/students_edges_all.csv'
...
for byType in range(3):
    ...
    for line in stats:
        if line[1] == '1':
            if line[byType] not in percentile_list[0]:
                percentile_list[0][int(line[byType])] = 1
            else:
                percentile_list[0][int(line[byType])] += 1

        elif line[1] == '2':
            if line[byType] not in percentile_list[1]:
                percentile_list[1][int(line[byType])] = 1
            else:
                percentile_list[1][int(line[byType])] += 1
    ...
    for gender in range(2):
        ...
        for k, v in sorted_percentile_list[gender].items():
            ...
            sum = sum + v
        OUTPUT_DIR = '../dataset_indegree/sorted_file_with_{}_{}.csv'.format(gender, byType)
