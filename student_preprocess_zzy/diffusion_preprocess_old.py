...
for i in range(len(post_type)):
    sorted_intensity_list[i] = sorted(hindex_list[i].items(), key = lambda d : d[1], reverse = True)
...
for i in range(3):
    ...
    #insert 0
    for j in id_dict:
        if j not in hindex_list[i]:
            line2write.append((id_dict[j], gender_dict[j], 0))

    OUTPUT_DIR = 'indegree/students_intensity_{}.csv'.format(types[i])
    with open(OUTPUT_DIR, 'w') as writefile:
                writer = csv.writer(writefile)
                for line in line2write:
                    writer.writerow(line)
