...
#users_indegree_test <- data[,.(indegree = n_distinct(source_id)),by = .(gender_target,target_id,week)]
...
groupBy_user_rec_intensity_rank <- groupBy_user_rec_intensity%>%mutate(rank = rank(-sum_indegree, ties.method = "min"))%>%mutate(percentage = rank/nrow(.))
...
p <- paste("indegree_",i,"_interaction_percentage_ratio.jpg")
