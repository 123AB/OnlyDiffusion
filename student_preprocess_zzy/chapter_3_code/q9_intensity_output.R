...
for(i in rec_intensity_percentage){
  ...
  groupBy_user_rec_intensity_rank_original <- groupBy_user_rec_intensity%>%mutate(rank = rank(-sum_rec_intensity, ties.method = "min"))%>%mutate(percentage = rank/nrow(.))
  ...
  rec_intensity_total_percent_test_m <- round(rec_intensity_total_row_m * 0.1)
...
#  l_f <-data.frame(week = numeric(),topMales = numeric(),topPopulation = numeric())
#  col_l <- c("week", "topMales", "topPopulation")
#  col_f <- c("week", "topFemales", "topPopulation")
  ...
  for(w in week_keeper){
    ...
    groupBy_user_at_week_rank <- user_at_week %>% filter(target_id %in% (rec_intensity_percent_ByPopulation_test$target_id))
...
#  ggplot()+geom_line(data = l1,aes(x = week,y = topPopulation,colour = "top 0.1% "),size=1)+
#    geom_line(data = l2,aes(x = week,y = topPopulation,colour = "top 1%"),size=1)+
#    geom_line(data = l3,aes(x = week,y = topPopulation,colour ="top 5%"),size=1) + 
#    geom_line(data = l4,aes(x = week,y = topPopulation,colour = "top 10%"),size=1)+
#    scale_colour_manual("",values = c("top 01%" = "red",
#                                      "top 1%" = "blue", "top 5%" = "green", "top 10%" = "purple"))+
...
p <- paste("female_intensity_",i,"_interaction_percentage.jpg")

ggplot()+geom_line(data = l5,aes(x = week,y = topPopulation,colour = "top 0.1%"),size=1)+
  geom_line(data = l6,aes(x = week,y = topPopulation,colour = "top 1%"),size=1)+
  geom_line(data = l7,aes(x = week,y = topPopulation,colour ="top 5%"),size=1) + 
  geom_line(data = l8,aes(x = week,y = topPopulation,colour = "top 10%"),size=1)+
  scale_colour_manual("",values = c("top 0.1%" = "black",
                                    "top 1%" = "blue", "top 5%" = "green", "top 10%" = "purple"))+
  xlab("week")+ylab("percentage")+
  theme(text=element_text(size=13, family="Comic Sans MS"))
ggsave(p)
