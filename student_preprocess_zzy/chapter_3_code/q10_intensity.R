...
path = paste0("D:\\R_test\\social\\z_master_thesis\\dataset_whole\\dataset_", criteria_vec[i], "\\q5_output_stay_top_intensity.csv")
...
for(i in rec_intensity_percentage){
  ...
  get_average_female <- function(weekly = weekly_gender_people_group_females2){
    weekly_prob_top1 <- weekly[order(week), .(n = .N, reach_top_week = sum(gender_target == "F")/sum(target_id == target_id)),by = .(target_id)]%>%
