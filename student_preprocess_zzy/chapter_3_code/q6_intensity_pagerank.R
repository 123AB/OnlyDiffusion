...
#i = 0.05
  ...
  groupBy_user_rec_intensity_rank <-  groupBy_user_rec_intensity%>%  filter(target_id %in% (rec_intensity_percent_ByPopulation_test$user_id))
  
  #print(groupBy_user_rec_intensity_rank)
  ...
  #w = 70  
    ...
    if(dim(groupBy_user_rec_intensity_rank_week)[1] != 0){
      
    #rec_intensity_total_row2 <- nrow(user_at_week)

    #rec_intensity_total_percent2 <- round(rec_intensity_total_row2 * i)
      #user_at_week <- filter(user_at_week, percentage<=i)
      
      user_pagerank_rank <- user_at_week %>%mutate(rank = rank(-rec_intensity, ties.method = "min"))%>%mutate(percentage = rank/nrow(.))
      
      user_pagerank_rank<- filter(user_pagerank_rank, percentage<=i)
      #rec_intensity_percent_ByPopulation_2 <- filter(user_pagerank_rank, percentage<=i)
    
      rec_intensity_percent_ByPopulation <- groupBy_user_rec_intensity_rank_week%>% filter((target_id %in% user_pagerank_rank$target_id))
  ...
  #user_rec_intensity_temp <- rec_intensity_percent_ByPopulation
    ...
    if (empty(rec_intensity_percent_ByPopulation)){
     # print("i come here hahaha")
      ...
      user_rec_intensity_temp_2 <- X %>%  filter(user_id %in% (rec_intensity_percent_ByPopulation$target_id))
