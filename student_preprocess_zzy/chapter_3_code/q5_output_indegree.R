...
peak <- data_filter3[, length(which(indegree >= (max(indegree) - max(indegree)*0.10))),by = .(target_id)]
...
write.csv(combine_data, "D:\\R_test\\social\\z_master_thesis\\q7_output_stay_top_indegree.csv", row.names = FALSE)
