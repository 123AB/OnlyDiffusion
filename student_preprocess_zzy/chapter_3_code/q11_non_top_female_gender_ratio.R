...
for(i in rec_intensity_percentage){
  ...
  rec_intensity_total_row <- nrow(m_groupBy_user_rec_intensity_rank)
  
  
  rec_intensity_total_percent_test <- round(rec_intensity_total_row * i)
  
  rec_intensity_percent_ByPopulation_test <- m_groupBy_user_rec_intensity_rank%>% filter(rank %in% (1:rec_intensity_total_percent_test)) 
