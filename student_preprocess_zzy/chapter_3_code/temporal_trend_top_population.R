...
get_top_population <- temp_users_rec_intensity%>%mutate(rank = rank(-rec_intensity, ties.method = "min"))%>%mutate(percentage = rank/nrow(.))
get_top_population_intensity <- sqldf("SELECT *, DENSE_RANK() OVER(ORDER BY rank) 
AS IntensityRank
FROM get_top_population")
real_top_population_intensity <- distinct(get_top_population_intensity, target_id, .keep_all = TRUE)
intensity_total_row <- nrow(real_top_population_intensity)

## Duration analysis top 0.1 percent intensity
intensity_total_percent1 <- round(intensity_total_row * 0.001)


sql_intensity1_total_percent <- paste("SELECT * FROM real_top_population_intensity limit",intensity_total_percent1,collapse = "+")
...
females_in_top1_intensity <- top1_population_intensity[gender_target == "F",]
females_in_top1_intensity_row <- nrow(females_in_top1_intensity)
...
top1_males_intensity <- males_in_top1_intensity


top1_females_intensity <- females_in_top1_intensity
...
ggplot()+geom_line(data = duration_all_males,aes(x = week,y = sum_intensity,colour = "male_group_intensity_1"),size=1)+
  ...
  geom_line(data = duration_all_males_average,aes(x = week,y = average_intensity,colour = "male_group1_avr"),size=0.5)+
  geom_point(data = duration_all_males_average,aes(x = week,y = average_intensity,colour = "male_group1_avr"),size=0.5)+
  geom_line(data = duration_all_females_average,aes(x = week,y = average_intensity,colour = "female_group1_avr"),size=0.5)+
  geom_point(data = duration_all_females_average,aes(x = week,y = average_intensity,colour = "female_group1_avr"),size=0.5)+
...
intensity_total_percent5 <- round(intensity_total_row * 0.01)


sql_intensity5_total_percent <- paste("SELECT * FROM real_top_population_intensity limit",intensity_total_percent5,collapse = "+")

top5_population_intensity <- sqldf(sql_intensity5_total_percent)
setDT(top5_population_intensity)
males_in_top5_intensity <- top5_population_intensity[gender_target == "M",]
males_in_top5_intensity_row <- nrow(males_in_top5_intensity)

females_in_top5_intensity <- top5_population_intensity[gender_target == "F",]
females_in_top5_intensity_row <- nrow(females_in_top5_intensity)

top5_males_intensity <- males_in_top5_intensity

top5_females_intensity <- females_in_top5_intensity
...
intensity_total_percent10 <- round(intensity_total_row * 0.05)


sql_intensity10_total_percent <- paste("SELECT * FROM real_top_population_intensity limit",intensity_total_percent10,collapse = "+")

top10_population_intensity <- sqldf(sql_intensity10_total_percent)
setDT(top10_population_intensity)
males_in_top10_intensity <- top10_population_intensity[gender_target == "M",]
males_in_top10_intensity_row <- nrow(males_in_top10_intensity)

females_in_top10_intensity <- top10_population_intensity[gender_target == "F",]
females_in_top10_intensity_row <- nrow(females_in_top10_intensity)

top10_males_intensity <- males_in_top10_intensity

top10_females_intensity <- females_in_top10_intensity
...
ggplot()+geom_line(data = duration_top10_males,aes(x = week,y = sum_intensity,colour = "male_group_intensity_10"),size=1)+
  ...
  geom_line(data = duration_top10_males_avr,aes(x = week,y = average_intensity,colour = "male_10_avr"),size=0.5)+
  geom_point(data = duration_top10_males_avr,aes(x = week,y = average_intensity,colour = "male_10_avr"),size=0.5)+
  geom_line(data = duration_top10_females_avr,aes(x = week,y = average_intensity,colour = "female_10_avr"),size=0.5)+
  geom_point(data = duration_top10_females_avr,aes(x = week,y = average_intensity,colour = "female_10_avr"),size=0.5)+
  geom_line(data = duration_top10_males_avr_per_m,aes(x = week,y = per_m_average_intensity,colour = "male_10_avr_per_m"),size=0.5)+
  geom_line(data = duration_top10_females_avrper_fm,aes(x = week,y = per_fm_average_intensity,colour = "female_10_avr_per_m"),size=0.5)+
...
########################################################
########################################################
##top percent indegree
temp_users_rec_indegree <- users_indegree
get_top_population_in <- temp_users_rec_indegree%>%mutate(rank = rank(-indegree, ties.method = "min"))%>%mutate(percentage = rank/nrow(.))
get_top_population_indegree <- sqldf("SELECT *, DENSE_RANK() OVER(ORDER BY rank) 
AS IndegreeRank
FROM get_top_population_in")
real_top_population_indegree <- distinct(get_top_population_indegree, target_id, .keep_all = TRUE)
indegree_total_row <- nrow(real_top_population_indegree)

## Duration analysis top 0.1 percent indegree
indegree_total_percent01 <- round(indegree_total_row * 0.001)


sql_indegree01_total_percent <- paste("SELECT * FROM real_top_population_indegree limit",indegree_total_percent01,collapse = "+")

top01_population_indegree <- sqldf(sql_indegree01_total_percent)
setDT(top01_population_indegree)
males_in_top01_indegree <- top01_population_indegree[gender_target == "M",]
males_in_top01_indegree_row <- nrow(males_in_top01_indegree)

females_in_top01_indegree <- top01_population_indegree[gender_target == "F",]
females_in_top01_indegree_row <- nrow(females_in_top01_indegree)

top01_males_indegree <- males_in_top01_indegree


top01_females_indegree <- females_in_top01_indegree
...
get_duration <- function(weekly = weekly_gender_people_group_males){
  weekly_prob_top01 <- weekly[order(week), .(n = .N, sum_indegree = sum(indegree)),by = .(week)]%>%
    .[, .(sum_indegree) ,by = .(week)]%>%.[,type := "males_group_indegree_01",]
...
get_duration_female <- function(weekly = weekly_gender_people_group_females){
  weekly_prob_top01 <- weekly[order(week), .(n = .N, sum_indegree = sum(indegree)),by = .(week)]%>%
    .[, .(sum_indegree) ,by = .(week)]%>%.[,type := "females_group_indegree_01",]
...
get_duration_avr_female <- function(weekly = weekly_gender_people_group_females){
  weekly_prob_top01_avr_tep <- weekly[order(week), .(n = .N, average_indegree = sum(indegree)/288)]
  ave_tep_data <- as.numeric(unlist(weekly_prob_top01_avr_tep[1,2]))
  
  weekly_prob_top01_avr <- weekly[order(week), .(n = .N, average_indegree = ave_tep_data),by = .(week)]%>%
    .[, .(average_indegree),by = .(week)]%>%.[,type := "female_01_indegree_avr",]
  
  
  weekly_prob_combined <- rbind(weekly_prob_top01_avr)
  return(weekly_prob_combined)
}
...
get_duration_avr_male <- function(weekly = weekly_gender_people_group_males){
  ...
  weekly_prob_top01_avr <- weekly[order(week), .(n = .N, average_indegree = ave_tep_data),by = .(week)]%>%
    .[, .(average_indegree),by = .(week)]%>%.[,type := "male_01_indegree_avr",]
...
ggplot()+geom_line(data = duration_top01_males,aes(x = week,y = sum_indegree,colour = "males_group_indegree_01"),size=1)+
  geom_point(data = duration_top01_males,aes(x = week,y = sum_indegree,colour = "males_group_indegree_01"),size=3)+
  geom_line(data = duration_top01_females,aes(x = week,y = sum_indegree,colour ="females_group_indegree_01"),size=1) + 
  geom_point(data = duration_top01_females,aes(x = week,y = sum_indegree,colour = "females_group_indegree_01"),size=3)+
  geom_line(data = duration_all_males_average,aes(x = week,y = average_indegree,colour = "male_01_indegree_avr"),size=0.5)+
  geom_point(data = duration_all_males_average,aes(x = week,y = average_indegree,colour = "male_01_indegree_avr"),size=0.5)+
  geom_line(data = duration_all_females_average,aes(x = week,y = average_indegree,colour = "female_01_indegree_avr"),size=0.5)+
  geom_point(data = duration_all_females_average,aes(x = week,y = average_indegree,colour = "female_01_indegree_avr"),size=0.5)+
  scale_colour_manual("",values = c("males_group_indegree_01" = "red","females_group_indegree_01" = "green",
                                    "male_01_indegree_avr" = "blue","female_01_indegree_avr" = "yellow"))+
...
########################################################
########################################################
## Duration analysis top 1 percent indegree
indegree_total_percent1 <- round(indegree_total_row * 0.01)


sql_indegree1_total_percent <- paste("SELECT * FROM real_top_population_indegree limit",indegree_total_percent1,collapse = "+")

top1_population_indegree <- sqldf(sql_indegree1_total_percent)
setDT(top1_population_indegree)
males_in_top1_indegree <- top1_population_indegree[gender_target == "M",]
males_in_top1_indegree_row <- nrow(males_in_top1_indegree)

females_in_top1_indegree <- top1_population_indegree[gender_target == "F",]
females_in_top1_indegree_row <- nrow(females_in_top1_indegree)

top1_males_indegree <- males_in_top1_indegree


top1_females_indegree <- females_in_top1_indegree
...
ggplot()+geom_line(data = duration_top1_males,aes(x = week,y = sum_indegree,colour = "males_group_indegree_1"),size=1)+
  ...
  geom_line(data = duration_all_males_average,aes(x = week,y = average_indegree,colour = "male_1_indegree_avr"),size=0.5)+
  geom_point(data = duration_all_males_average,aes(x = week,y = average_indegree,colour = "male_1_indegree_avr"),size=0.5)+
  geom_line(data = duration_all_females_average,aes(x = week,y = average_indegree,colour = "female_1_indegree_avr"),size=0.5)+
  geom_point(data = duration_all_females_average,aes(x = week,y = average_indegree,colour = "female_1_indegree_avr"),size=0.5)+
...
########################################################
## Duration analysis top 5 percent indegree
indegree_total_percent5 <- round(indegree_total_row * 0.05)


sql_indegree5_total_percent <- paste("SELECT * FROM real_top_population_indegree limit",indegree_total_percent5,collapse = "+")

top5_population_indegree <- sqldf(sql_indegree5_total_percent)
setDT(top5_population_indegree)
males_in_top5_indegree <- top5_population_indegree[gender_target == "M",]
males_in_top5_indegree_row <- nrow(males_in_top5_indegree)

females_in_top5_indegree <- top5_population_indegree[gender_target == "F",]
females_in_top5_indegree_row <- nrow(females_in_top5_indegree)

top10_males_indegree <- males_in_top5_indegree


top10_females_indegree <- females_in_top5_indegree
##########################################################
...
ggplot()+geom_line(data = duration_top5_males,aes(x = week,y = sum_indegree,colour = "males_group_indegree_5"),size=1)+
  ...
  geom_line(data = duration_all_males_average,aes(x = week,y = average_indegree,colour = "male_5_indegree_avr"),size=0.5)+
  geom_point(data = duration_all_males_average,aes(x = week,y = average_indegree,colour = "male_5_indegree_avr"),size=0.5)+
  geom_line(data = duration_all_females_average,aes(x = week,y = average_indegree,colour = "female_5_indegree_avr"),size=0.5)+
  geom_point(data = duration_all_females_average,aes(x = week,y = average_indegree,colour = "female_5_indegree_avr"),size=0.5)+
  scale_colour_manual("",values = c("males_group_indegree_5" = "red","females_group_indegree_5" = "green",
                                    "male_5_indegree_avr" = "blue","female_5_indegree_avr" = "yellow"))+  xlab("week")+ylab("sum_indegree")+
  theme(text=element_text(size=13, family="Comic Sans MS"))
