...
seeds = [1, 2, 5, 10, 20, 50, 75, 100, 125, 150, 175, 200]
types = ["like", "comment"]
...
FILE_DIR = '../only_post/students_edges_all.csv';
...
for ratio in range(11):
    ...
    for s in seeds:
        ...
        #FILE_DIR = 'indegree_data/students_indegree_comment_{}.csv'
        #FILE_DIR = 'indegree_data_twomonth/students_indegree_{}_{}.csv'
        #FILE_DIR = 'indegree_data_twoweek/students_indegree_{}_{}.csv'
        FILE_DIR = 'indegree_data_week/students_indegree_{}_{}.csv'


        #FILE_WRITE = '../../dataset/seed_selection/remove1interaction/diversity/diversity-{}-{}-{}.csv'
        #FILE_WRITE = '../../../student_preprocess/indegree_temporal/indegree_diversity_seed_10/diversity-{}-{}-{}-{}.csv'

        #FILE_WRITE = 'indegree_diversity_seed/diversity-{}-{}-{}-{}.csv'
        #FILE_WRITE = 'indegree_diversity_seed_twomonth/diversity-{}-{}-{}-{}.csv'
        FILE_WRITE = 'indegree_diversity_seed_oneweek/diversity-{}-{}-{}-{}.csv'
        #FILE_WRITE = 'indegree_diversity_seed_twoweek/diversity-{}-{}-{}-{}.csv'

        for type in types:

            for byType in range(999):
                if (os.path.exists(FILE_DIR.format(type,byType)) == True):

                    user_list = []
                    fs = round(s * fr, 0)
                    ms = round(s * mr, 0)
                    with open(FILE_DIR.format(type,byType), 'r') as read_file:
                        for line in read_file:
                            token = line.strip().split(' ')
                            userId = token[0]
                            #userId = eval(token[0])
                    ...
                    #seedfile = 'indegreec'
                    #with open(FILE_WRITE.format(type, s, ratio/100,byType), 'w+b') as write_file:
                    with open(FILE_WRITE.format(type, s, ratio/10,byType), 'w+b') as write_file:
