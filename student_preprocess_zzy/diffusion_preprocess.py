...
with open(file_edge, 'r') as read_file:
    ...
    for line in read_file:
        ...
        #timeStamp = math.ceil(int(token[3])/1209600) - 732  ##this is per two week
...
#########################################################
        line2write = []
        OUTPUT_DIR = 'all/students_edges_all_{}.csv'
        ...
        for edge in keys:
            ...
            line2write.append((
                              id_dict[usr2], gender_dict[usr2], id_dict[usr1], gender_dict[usr1], comment_num, like_num,
                              tag_num, interact_num))

        with open(OUTPUT_DIR.format(weeknum), 'w',newline='\n') as writefile:
            #writer = csv.writer(writefile,delimiter=" ")
