...
for testcase in testcases:
    ...
    for byType in range(2):
        ...
        mr_list = []
        with open("../../../../fairness/dataset/discount_factor_train/weight/weight_hindex_{}.txt".format(byType), 'r') as read_file:
            for line in read_file:
                token = line.split(',')
                mr_list.append(token[0])
        
        for idx, ratio in enumerate(mr_list):
            result[byType] = H_index({**female_user[byType], **male_user[byType]}, float(ratio))
            OUTPUT_DIR = '../../../dataset/seed_selection/{}/hi-index_test/HIindex_sort_{}_{}.csv'.format(testcase, byType, idx/10)
