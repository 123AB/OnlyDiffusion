...
'''
def count_N_target(neighbor_degree_list, index):
    count = 0
    for num in neighbor_degree_list:
        if num >= index:
            count += 1
...
def TARGET_hi_index(hi_index_list, Nf_hi_index_list,target_ratio):
    ...
    for usr in hi_index_list:
        if(usr in Nf_hi_index_list):
            target_hi_index_list[usr] = abs((Nf_hi_index_list[usr]/hi_index_list[usr]) - target_ratio)
...
file_degree = 'fb_degree_like.csv'
file_edge_list = 'fb_edge_list_like.csv'
...
line2write = []
OUTPUT_DIR = 'like_hi_index.csv'

hi_index_list = HI_index(degree_list, edge_list)
nf_hi_index_list = Nf_hi_index(degree_list,edge_list_female)
...
for ratio in range(11):
    target_ratio = ratio/10;
    target_hi_index_list = TARGET_hi_index(hi_index_list,nf_hi_index_list,target_ratio)
    target_sorted_degree_list = sorted(target_hi_index_list.items(), key=lambda d: int(d[1]), reverse=True)
...
#print('#graph size: ', len(target_hi_index_list))
